...
#include <stdio.h>
// run this :
// gcc main_test_copy.c ../libraries/libft/libft.a ../libraries/libft/ft_printf/libftprintf.a get_next_line_utils.c get_next_line.c
...
int	check_assets(char **table)
{
	...
	i = 0;
	//parcourir le tableau et compter +1 pour chaque fois qu'on tombe sur la letre
	//faire comme inter, a revoir
	while (table[i])
	{
		j = 0;
		while (table[i][j])
	...
	//conditions : avoir un seul P un seul E et le reste doit etre soit 0 soit 1 only
	if (ass.P != 1 || ass.E != 1 || ass.C < 1)
		return (0);
}
